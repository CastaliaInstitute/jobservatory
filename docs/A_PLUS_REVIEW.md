...
|---|---:|---|
| Research validity | B+ | Unit of analysis, declared source universe, three-ATS/24-sector coverage, source API versions, concentration diagnostics, and machine-readable expansion targets are explicit. Thirty-four selected employers remain a curated cohort that cannot support labor-market prevalence claims; all five coverage-design checks currently fail visibly. |
...
| O*NET normalization | B− | Conservative occupation candidates cover 950+ observations. Exact occupation-linked O*NET 30.3 software examples normalize 1,390+ observed skill mentions on 660+ listings, while inherited essential/transferable profiles are explicitly separated from listing facts. Exact counts are machine-readable; mappings remain unreviewed and only six occupations are profiled. |
...
| Reliability/automation | B+ | Source count gates, strict public JSON Schema validation, build, lint, baseline ML evaluation, tests, and data refresh run in CI. A content-addressed BM25 baseline now runs as a versioned Pages Function with health/lineage checks, bounded inputs, isolate caches, structured logs, runtime packaging tests, and separate live latency/contract/cost evidence. The latest 80-request production run recorded zero errors or contract failures, 22 ms application p95, and 289 ms client-observed p95; earlier post-deploy runs also expose edge-propagation mixing. Scheduled/manual refreshes collect; ordinary pushes validate the committed snapshot. Automatic Cloudflare deployment and durable log/alert evidence remain absent. |
