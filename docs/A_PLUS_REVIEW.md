...
| Reliability/automation | B+ | Source count gates, strict public JSON Schema validation, build, lint, baseline ML evaluation, tests, and data refresh run in CI. A content-addressed BM25 baseline now runs as a versioned Pages Function with health/lineage checks, bounded inputs, isolate caches, structured logs, runtime packaging tests, and separate live latency/contract/cost evidence. The latest warm 80-request production run recorded zero errors or contract failures, sub-millisecond application p95, and 169 ms client-observed p95; earlier post-deploy runs also expose edge-propagation mixing. CI still does not deploy the direct-upload project after a data commit, and durable log/alert evidence is absent. |
